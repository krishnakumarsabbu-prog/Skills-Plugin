  ...
  return (
    <aside className="sidebar w-[220px] flex flex-col shrink-0 relative">
      ...
      <div className="flex items-center gap-3 px-5 py-5 border-b border-white/5">
        ...
        <div>
          <div className="text-[11px] font-bold text-white leading-tight tracking-wider">DCT COPILOT</div>
        ...
        {navItems.map((item) => (
          <button
            ...
            className={`nav-item w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 cursor-pointer border-none outline-none
              ${activeView === item.id ? 'nav-active' : 'text-slate-400 hover:text-white'}`}
          >
      ...
      {/* Source Location */}
      <div className="mx-3 mb-3 p-3 rounded-xl glass-card border border-white/5">
        <div className="text-[9px] font-bold text-slate-500 tracking-widest uppercase mb-2">Source Location</div>
        <div className="flex items-center gap-2">
          <span className="text-slate-400 text-[10px] font-mono truncate flex-1">~/.copilot-master/.github/skills</span>
          <button className="text-slate-500 hover:text-blue-400 transition-colors shrink-0 border-none bg-transparent cursor-pointer p-0">
